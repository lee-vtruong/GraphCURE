...
`0.57535/0.55507`. This is the confirmatory zero-leakage result used for the
primary claim. A post-hoc threshold sensitivity analysis reaches
`0.57821/0.56166` at $\tau=0.60$, but the recorded grid identifies that operating
point as the test peak; it is therefore exploratory evidence for routing
potential, not the main SOTA result. Relative to AMuFC arXiv v2, the
confirmatory B18-A gains are `+0.02935` Accuracy and `+0.01507` Macro-F1.
