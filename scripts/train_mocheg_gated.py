...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument("--root", type=Path, default=Path("data/processed/mocheg_gold_embeddings")); ap.add_argument("--out", type=Path, default=Path("outputs/mocheg_gated")); ap.add_argument("--epochs",type=int,default=20); ap.add_argument("--batch-size",type=int,default=256); ap.add_argument("--device",default="cuda"); ap.add_argument("--seed",type=int,default=42); a=ap.parse_args()
    ...
    for ep in range(1,a.epochs+1):
        ...
        for c,e,m,y in loader(tr,True):
            c,e,m,y=c.to(dev),e.to(dev),m.to(dev),y.to(dev); opt.zero_grad(); logits,_=model(c,e,m); loss=loss_fn(logits,y); loss.backward(); opt.step(); total+=loss.item()*len(y)
        model.eval(); ys=[]; ps=[]
        with torch.no_grad():
