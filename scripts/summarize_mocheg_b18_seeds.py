...
def main() -> None:
    ...
    parser.add_argument("--control-roots", type=Path, nargs="+", required=True, help="Matched control directories")
    ...
    if len(args.candidate_roots) != len(args.control_roots):
    ...
    ctrl_by_seed = [load_seed_predictions(p) for p in args.control_roots]

    # Find common IDs across all seeds
    common_ids = set(cand_by_seed[0].keys())
    for c_dict in cand_by_seed + ctrl_by_seed:
