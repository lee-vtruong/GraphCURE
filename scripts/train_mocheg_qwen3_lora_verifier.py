...
class QwenVerifierDataset(Dataset):
    def __init__(self, manifest: Path, retrieval: Path, corpus: Path,
                 ...
                 inject_train_gold: bool = False, limit: int = 0) -> None:
...
def main() -> None:
    ...
    train=QwenVerifierDataset(a.manifest_root/"train.jsonl",a.retrieval_root/"train.jsonl",a.raw_root/"train"/"Corpus2.csv",a.top_k,a.max_evidence_chars,not a.no_train_gold_injection,a.limit_train)
    val=QwenVerifierDataset(a.manifest_root/"val.jsonl",a.retrieval_root/"val.jsonl",a.raw_root/"val"/"Corpus2.csv",a.top_k,a.max_evidence_chars,False,a.limit_val)
    ...
    stale=0; history=[{"epoch":0,"train_loss":None,**initial}]
    ...
    temporary.replace(target); optimizer.zero_grad(set_to_none=True)
    for epoch in range(1,a.epochs+1):
        ...
        metrics,_=evaluate(model,val_loader,answer_ids,device); row={"epoch":epoch,"train_loss":running/len(train_loader),**metrics}; history.append(row); print(json.dumps(row),flush=True)
